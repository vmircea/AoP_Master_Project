...
public interface UserDataRepository extends JpaRepository<UserData, String> {
    ...
    @Query("select case when count(userData)> 0 then true else false end from UserData userData where lower(userData.username) = lower(:username)")
    public boolean usernameExists(@Param("username") String username);

    @Query("select userData from UserData userData where lower(userData.username) = lower(:username)")
